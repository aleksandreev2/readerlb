...
internal const val MAX_EPUB_EXTRACTED_IMAGES_BYTES = 128L * 1024 * 1024
...
internal fun copyEpubSource(input: InputStream, target: File, maxBytes: Long = MAX_EPUB_SOURCE_BYTES) {
    var total = 0L
    try {
        target.outputStream().buffered().use { output ->
            ...
            while (true) {
                ...
                if (count < 0) break
                total += count
                if (total > maxBytes) throw EpubLimitException("Исходный EPUB слишком большой (лимит ${maxBytes / 1024 / 1024} МБ)")
